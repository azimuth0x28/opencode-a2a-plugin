...
- `ContextScout` — finds WHERE code lives and extracts implementation patterns
- `CodeReviewer` — analyzes HOW integration points work and traces data flow
- `ExternalScout` — strategic web research with citations and gap analysis
...
### Agent 1: `ContextScout`

Finds WHERE code lives and extracts implementation patterns.

Use Task tool with `subagent_type="ContextScout"`:
...
- [ ] Both agents (`ContextScout` and `CodeReviewer`) launched in parallel and completed
...
**Use Task tool with `subagent_type="ExternalScout"`:**
...
- [ ] `ExternalScout` agent launched and completed
...
**For complex features with multiple integration points**, use `CodeReviewer` to trace how existing architecture works at the integration points identified in Phase 2:
...
**OUTPUT_PATH**: `.opencode/PRPs/plans/{kebab-case-feature-name}.plan.md`

Create directory if needed: `mkdir -p .opencode/PRPs/plans`
...
<output>
**OUTPUT_FILE**: `.opencode/PRPs/plans/{kebab-case-feature-name}.plan.md`
...
**File**: `.opencode/PRPs/plans/{feature-name}.plan.md`
...
- {Pattern 1 from ContextScout/CodeReviewer with file:line}
- {Pattern 2 from ContextScout/CodeReviewer with file:line}
...
**Next Step**: To execute, run: `/prp-implement .opencode/PRPs/plans/{feature-name}.plan.md`
````
...
- [ ] All patterns from `ContextScout` and `CodeReviewer` documented with file:line references
...
<success_criteria>
**CONTEXT_COMPLETE**: All patterns, gotchas, integration points documented from actual codebase via `ContextScout` and `CodeReviewer` agents
