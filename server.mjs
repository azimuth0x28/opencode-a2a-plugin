...
const agentCard = {
	...
	description:
		process.env.A2A_AGENT_DESC || "OpenCode AI coding assistant as A2A agent",
...
const agentExecutor = {
	...
	execute: async function (requestContext, eventBus) {
		...
		console.log(
			`[A2A] Processing message ${userMessage.messageId} for task ${taskId}`,
		);
		...
		try {
			...
			// Call OpenCode
			console.log(
				`[A2A] Calling OpenCode with: "${userText.substring(0, 50)}..."`,
			);
			const response = await processWithOpenCode(userText);
			console.log(
				`[A2A] OpenCode responded: "${response.substring(0, 50)}..."`,
			);
