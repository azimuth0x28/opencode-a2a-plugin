...
// Store config and getClient for use in tools
let pluginConfig: A2APluginConfig;
let getClient: () => Promise<any>;

/**
 * Initialize the tools with plugin context
 */
...
export const a2a_send = tool({
  description: "Send a message to an A2A agent. Uses default URL from config (http://localhost:4000) if agentUrl not provided.",
  args: {
    message: tool.schema.string().describe("Message to send to the A2A agent"),
    agentUrl: tool.schema.string().optional().describe("Optional. A2A agent URL. Defaults to http://localhost:4000 from config"),
  },
  execute: async (args) => {
    const agentUrl = args.agentUrl || pluginConfig?.agentUrl;
...
export const a2a_discover = tool({
  ...
  execute: async (args) => {
    try {
      ...
      output += `**Protocol Version:** ${(card as any).protocolVersion}\n`;
      output += `**URL:** ${card.url}\n\n`;
      ...
      return `Error: ${error instanceof Error ? error.message : "Failed to fetch agent card"}`;
    }
  },
});

// Tool: Get task status
export const a2a_task_status = tool({
  ...
  execute: async (args) => {
    ...
    try {
      ...
      return `Error: ${error instanceof Error ? error.message : "Failed to get task status"}`;
    }
  },
});

// Tool: Cancel task
export const a2a_cancel = tool({
  ...
  execute: async (args) => {
    ...
    try {
      ...
      return `Error: ${error instanceof Error ? error.message : "Failed to cancel task"}`;
    }
  },
});
