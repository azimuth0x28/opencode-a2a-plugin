 ...
 */
export const log = {
  debug: async (module: string, message: string, extra?: Record<string, unknown>) => {
    await createLogger(module).log({
      body: { service: `a2a-plugin.${module}`, level: "debug", message, extra },
    });
  },
  info: async (module: string, message: string, extra?: Record<string, unknown>) => {
    await createLogger(module).log({
      body: { service: `a2a-plugin.${module}`, level: "info", message, extra },
    });
  },
  warn: async (module: string, message: string, extra?: Record<string, unknown>) => {
    await createLogger(module).log({
      body: { service: `a2a-plugin.${module}`, level: "warn", message, extra },
    });
  },
  error: async (module: string, message: string, extra?: Record<string, unknown>) => {
    await createLogger(module).log({
